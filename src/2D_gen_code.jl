...
@generated function my_reconstruct(s, ::Val{D}, ::Val{B}, τ, boundary) where {D, B}
...
function myReconstruction(s::AbstractArray{T,3}, D,τ::DT,B=1,k=1,boundary=10, a=1,b=1) where {T, DT}
    Reconstruction{D*(2B+1)^2,T,DT}(my_reconstruct(s, Val{D}(), Val{B}(),τ,boundary), τ)
end
...
begin
    ...
    subplot(312)
    #Prediction
    s_pred = localmodel_stts(mat,3,1,p,1,1,10, 0,0)
